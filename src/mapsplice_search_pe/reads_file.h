...
public:
	...
	Reads_File(char* filename, bool _fasta, int segment_number)
	{
		rd_fs.open(filename);
		if( !rd_fs ) 
		{
	...
	Reads_File(char* filename, bool _fasta, bool pair_end, int segment_number)
	{
		rd_fs.open(filename);
		if( !rd_fs ) 
		{
	...
	size_t load_next_read_pairend(Read_Block& r_block)
	{
		...
		while(load_next_line())
		{
			char id_tmp[MAX_READID_LENGTH];
			int segment_no;
			sscanf(line.c_str(), "%*c%*[^~]%*[~]%*[^~]%*[~]%[^/]%*[/]%d",id_tmp, &segment_no);	
			if(r_block.read_id.empty())
			{
				r_block.read_id=id_tmp;
				if(segment_no!=1)
				{
					...
					{
						r_block.read_seq.push_back("A");
						r_block.read_quality.push_back("a");
					}
				}
			}
			else if(r_block.read_id!=id_tmp)
