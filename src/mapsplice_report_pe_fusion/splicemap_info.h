...
public:
	...
	void set_value_single(string line)
	{
		...
		char* p=jump_code_tmp;
		char jump_len_buf[MAX_JUMPCODE_DIGITS];
		int index=0;
		while(*p!='\0')
		{
			if(*p >= '0'&& *p <='9')
			{
				jump_len_buf[index]=*p;
				index++;
			}
			else
			{
				jump_len_buf[index]='\0';
				Jump_Code new_jump_code(atoi(jump_len_buf), *p);
				jump_code.push_back(new_jump_code);
				index=0;
			}
			p++;
		}
