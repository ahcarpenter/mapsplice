...
public:
	...
	int load_next_block(vector<Bwtmap_Info_Pe>& read_mapping_set) // load a block of junctions
	{
		read_mapping_set.clear();
		if(!load_next_line())
		{		
		...
		Bwtmap_Info_Pe first_bwt_info(line);
		read_mapping_set.push_back(first_bwt_info);
		line.clear();
		while(load_next_line())
		{
			Bwtmap_Info_Pe new_bwt_info(line);
			if(new_bwt_info.read_id==first_bwt_info.read_id)
			{
				read_mapping_set.push_back(new_bwt_info);
				line.clear();
