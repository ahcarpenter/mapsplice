...
int main(int argc, char* argv[])
{
	...
	int min_intron = (int) atoi(argv[2]);
	...
	if (ifs.is_open())
	{
		...
		while (!ifs.eof() )
		{
			...
			if (suffixst - prefixend - 1 <= min_intron)
			{
				++canon_count;
	...
	cout <<"in small deletion junction: "<<canon_count<<endl;

	cout <<"not in small deletion junction: "<<non_canon_count<<endl;
}