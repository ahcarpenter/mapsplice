/*    
 *    generate_bash_file_and_run.cpp		
 *    MapSplice
 *
...
int main(int argc, char* argv[])
{
	...
	string arguments = argv[4];

	string bin_dir = argv[5];
	...
	if (ifs.is_open())
	{
		while (!ifs.eof() )
		{
			...
			ofs << "python "<<bin_dir<<"/mapsplice_segments.py"<<" -o "<<abs_path<<relative_path<<"/output "
				/*<<"--max-hits 10 -X 4 -n 10 -w 50 -S fa -Q fq -L 25 --not-rem-temp" <<*/<<arguments <<" --pairend -c "<<abs_path<<relative_path
				<<" -u "<<abs_path<<folder_chr <<"extracted_reads_file"
				<<" -a "<<abs_path<<relative_path <<"/combined_sequence"
				<<" -B "<<abs_path<<relative_path<<"/index "<<" 2>"
				<<abs_path<<relative_path<<".log"<<endl;
	...
	//string assign_exe = "chmod 700 ";

	//assign_exe.append(bash_file);

	//system(assign_exe.c_str());

	//ofs.close();

	//string run_bash = bash_file;

	//system(run_bash.c_str());
}