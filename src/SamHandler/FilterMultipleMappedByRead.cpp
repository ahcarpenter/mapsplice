...
main(int argc, char** argv)
{
	...
	if (argc >= 9)
	{
		mps_samhandler.FilterMultipleMappedTagByRead(mps_sam, unique_mapped, filtered_multiple, junc_file, maxhits, stat_file, atof(argv[7]), atof(argv[8]), atof(argv[9]));
		//mps_samhandler.FilterMultipleMappedTag(filtered_multiple, junc_file, maxhits, atof(argv[6]), atof(argv[7]), atof(argv[8]));
	}
	else
	{
		mps_samhandler.FilterMultipleMappedTagByRead(mps_sam, unique_mapped, filtered_multiple, junc_file, maxhits, stat_file);
	}
